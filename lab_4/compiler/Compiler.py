...
class Compiler:
    ...
    def _get_stage_order(self, state: CompilerState) -> int:
        """Возвращает порядковый номер этапа"""
        order = {
            CompilerState.INITIALIZED: 0,
            CompilerState.LEXICAL_ANALYSIS: 1,
            CompilerState.SYNTAX_ANALYSIS: 2,
            CompilerState.OPTIMIZATION: 3,
            CompilerState.CODE_GENERATION: 4,
            CompilerState.COMPLETED: 5,
            CompilerState.ERROR: 6
        }
    ...
    def compile_to_stage(self, target_stage: CompilerState) -> bool:
        ...
        try:
            ...
            start_order = self._get_stage_order(self.state)
            target_order = self._get_stage_order(target_stage)
            ...
            print(f"Текущий этап: {self.state.value}")

            # Лексический анализ
            if target_order >= self._get_stage_order(CompilerState.LEXICAL_ANALYSIS) and self._tokens is None:
            ...
            if target_order >= self._get_stage_order(CompilerState.SYNTAX_ANALYSIS) and self._ast is None:
            ...
            # Оптимизация
            if target_order >= self._get_stage_order(CompilerState.OPTIMIZATION) and self._optimized_ast is None:
            ...
            # Генерация кода
            if target_order >= self._get_stage_order(CompilerState.CODE_GENERATION) and self._machine_code is None:
            ...
            # Завершено
            if target_order >= self._get_stage_order(CompilerState.COMPLETED):
